...
type ReceiveMsgReq struct {
	...
	FromUserName string `json:"from_user_name"`    // 消息的发送者
...
func WechatUploadAudioToOSS(mp3Path string) (resource_url string, err error) {
	...
	osskey := path.Join("self-wechat", strconv.FormatInt(utils.GetTimeZoneTime(config.Config.Cfg.TimeZone).UnixNano(), 10), mp3Path, mp3Name)
